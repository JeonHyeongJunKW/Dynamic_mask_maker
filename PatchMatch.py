...
def reconstruction(f, A, B):
    A_h = np.size(A, 0)
    A_w = np.size(A, 1)
    temp = np.zeros_like(A)##A의 형태
    for i in range(A_h):
        for j in range(A_w):
            temp[i, j, :] = B[f[i, j][0], f[i, j][1], :]#B의 형태에 해당되는값을 넣는다.
...
def initialization(A, B, p_size):
    ...
    B_w = np.size(B, 1)
    p = p_size // 2
    random_B_r = np.random.randint(p, B_h-p, [A_h, A_w])#하한값 p, 상한값 B_h-p ~ row(행의 값)
    random_B_c = np.random.randint(p, B_w-p, [A_h, A_w])#하한값 p, 상한값 B_w-p ~ col(열의 값)
    A_padding = np.ones([A_h+p*2, A_w+p*2, 3]) * np.nan
    A_padding[p:A_h+p, p:A_w+p, :] = A#패딩제외하고는 원래 정상적인값
    ...
    return f, dist, A_padding #랜덤값으로 초기화를 했다.
...
if __name__ == "__main__":
    ...
    img = cv2.resize(img,dsize=(int(img.shape[1]/3),int(img.shape[0]/2)),interpolation=cv2.INTER_AREA)
    ref = np.array(Image.open("./3.jpg"))
    ref = cv2.resize(ref, dsize=(int(ref.shape[1] / 3), int(ref.shape[0] / 2)), interpolation=cv2.INTER_AREA)
    print(ref.shape)
    p_size = 3
    itr = 5
    start = time.time()
    f = NNS(img, ref, p_size, itr)#이미지를 받아서
    end = time.time()
    print(end - start)
